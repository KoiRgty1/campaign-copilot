...
def generate_sms_copies(intent_label):
    try:
        ...
        completion = client.chat.completions.create(
            model="meta/llama-3.2-3b-instruct",
            messages=[
                {"role": "system", "content": "你是一个极其听话的电商运营助手。你只说纯中文，绝对不输出任何英文单词和代码符号。"},
                {"role": "user", "content": prompt}
            ],
            temperature=0.3, # 降低温度，让模型表现更稳定、更听话
            max_tokens=400,
            timeout=10.0
        )
        ...
        for line in lines:
            ...
            # 二次兜底过滤：如果 3B 模型还是吐了英文，直接在代码层把它干掉或替换
            line_str = line_str.replace(".register", "注册").replace("register", "注册").replace("redeem", "兑换")
        ...
        print(f"❌ 文案生成失败: {e}")
        return [
            f"【品牌名】新人专属好礼！关注的心仪商品正在限时特惠，专享满减券已到账！超多优惠等你来：xxxxxx 拒收请回复R",
            f"【品牌名】限时秒杀开启！专属您的惊喜回馈礼券已到账，挑选爆款立减！超多优惠等你来：xxxxxx 拒收请回复R",
            f"【品牌名】年中狂欢加码！全场运动爆款低至5折起，点击即刻挑选！超多优惠等你来：xxxxxx 拒收请回复R",
            f"【品牌名】专享无门槛券！感谢支持，专属于您的特权保价权益已生效！超多优惠等你来：xxxxxx 拒收请回复R",
            f"【品牌名】限时满减特惠！您购物车里的精选好物今晚截止优惠，速戳！超多优惠等你来：xxxxxx 拒收请回复R"
        ]
